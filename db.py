...
-- ===================================================================
-- PlainSight v2 / schema v4 data-layer groundwork
-- ===================================================================

-- Append-only panel snapshots. These are observations, not decode records:
-- multiple reruns for the same subject/date/channel are allowed and reads take
-- the newest created_at/id.
...
-- Manual execution facts used by decision/monitoring layers. This is not a
-- brokerage integration and does not claim realtime position truth.
...
-- Drift-backfill job bookkeeping: coverage, budget, write count, skips, errors.
...
-- Monitor feed items (SPEC F3). Append-only change summaries generated by
-- events-first monitoring scans. severity=high items are also pushed via SSE.
CREATE TABLE IF NOT EXISTS monitor_feed (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    item_id         TEXT    NOT NULL UNIQUE,       -- uuid for dedup / client id
    timestamp       TEXT    NOT NULL,
    subject         TEXT    NOT NULL,
    trigger         TEXT    NOT NULL,              -- earnings | rating | gap | manual | drift
...
CREATE INDEX IF NOT EXISTS idx_monitor_feed_subject_ts
    ON monitor_feed(subject, timestamp DESC);
CREATE INDEX IF NOT EXISTS idx_monitor_feed_severity_ts
    ON monitor_feed(severity, timestamp DESC);

-- Monitor scan runs (tracks each scan execution + budget).
CREATE TABLE IF NOT EXISTS monitor_scans (
    ...
    subjects_scanned INTEGER DEFAULT 0,
    credits_spent   INTEGER DEFAULT 0,
    credits_budget  INTEGER DEFAULT 80,
    skipped_json    TEXT,                          -- [{subject, reason}]
    feed_items_generated INTEGER DEFAULT 0,
    error           TEXT,
    created_at      TEXT    NOT NULL,
    completed_at    TEXT
);
CREATE INDEX IF NOT EXISTS idx_monitor_scans_created
    ON monitor_scans(created_at DESC);
"""
...
SCHEMA_VERSION = "4"  # v1 = original tables; v2 = Bet Card model + anchors; v3 = decode_detail + lineage; v4 = v2 panel/ledger/backfill groundwork
...
def card_from_row(conn: sqlite3.Connection, row: sqlite3.Row) -> BetCard:
    ...
    if "decode_detail_json" in cols:
        ...
        if dd is not None:
            card.decode_detail = _migrate_decode_detail_v4(dd)
    return card
...
def _project_v3_to_panel_cashflow(dd: dict) -> dict:
    """Best-effort v3 decode_detail -> v4 panel[0] projection.

    It preserves old fields and only adds a minimal cashflow observation wrapper.
    Unknown values stay explicit rather than invented.
    """
    metrics = {
        "primary_lens": (dd.get("primary_lens") or {}).get("lens"),
        "anchor_price": dd.get("anchor_price"),
        "narrative_premium": dd.get("narrative_premium"),
        "r2_band": dd.get("r2_band"),
        "primary_lens_result": dd.get("primary_lens"),
        "cross_validation": dd.get("cross_lenses") or [],
    }
    return {
        "channel": "cashflow",
        "horizon": "5y",
        "headline": "v3 card: projected cashflow panel",
        "quality": "medium" if dd.get("primary_lens") else "empty",
        "status": "ok" if dd.get("primary_lens") else "honest_empty",
        "metrics": metrics,
        "cost_credits": 0,
    }
...
def _migrate_decode_detail_v4(dd: dict) -> dict:
    """Idempotently normalize a persisted v3 decode_detail into the v4 superset."""
    ...
    if "reconciliation" not in dd:
        dd["reconciliation"] = {
            ...
            "term_structure": {
                "shape": "unknown",
                "note": "v3 card: no v4 term-structure reconciliation",
...
# ---------------------------------------------------------------------------
# DAO: v2 panel / position / backfill groundwork (schema v4)
# ---------------------------------------------------------------------------

def insert_panel_observation(
    conn: sqlite3.Connection,
    *,
    subject: str,
    as_of_date: str,
    ...
    """Append one panel observation row and return its id."""
    with conn:
        cur = conn.execute(
            """
            INSERT INTO panel_observations (
                subject, as_of_date, channel, horizon, metrics_json,
                quality, status, cost_credits, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                subject,
...
def _panel_observation_from_row(row: sqlite3.Row) -> dict:
    out = dict(row)
    out["metrics"] = _loads_json(out.pop("metrics_json"), {})
    return out
...
def list_panel_observations(
    ...
    *,
    ...
    """Return observation history newest first for a subject, optionally a channel."""
    sql = "SELECT * FROM panel_observations WHERE subject = ?"
    ...
    return [_panel_observation_from_row(r) for r in rows]


def get_latest_panel_observations(
    conn: sqlite3.Connection,
    subject: str,
    channel: str | None = None,
) -> list[dict]:
    """Return the latest observation per channel, or the one latest channel row."""
    if channel is not None:
        row = conn.execute(
            """
            SELECT * FROM panel_observations
            WHERE subject = ? AND channel = ?
            ORDER BY as_of_date DESC, created_at DESC, id DESC
            LIMIT 1
            """,
            (subject, channel),
        ).fetchone()
        return [_panel_observation_from_row(row)] if row is not None else []
    rows = conn.execute(
        """
        SELECT * FROM (
            SELECT p.*,
...
def record_position_ledger_event(
    conn: sqlite3.Connection,
    *,
    subject: str,
    source: str,
    ...
    """Append one manual position fact and return its id."""
    with conn:
        cur = conn.execute(
            """
            INSERT INTO position_ledger (
                subject, source, plan_card_id, side, quantity, weight_pct,
                avg_price, executed_at, status, note, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                subject,
...
def list_position_ledger_events(
    conn: sqlite3.Connection,
    *,
    ...
    sql = "SELECT * FROM position_ledger WHERE 1=1"
    params: list[Any] = []
    ...
    if plan_card_id is not None:
        sql += " AND plan_card_id = ?"
        params.append(plan_card_id)
    sql += " ORDER BY executed_at DESC, created_at DESC, id DESC"
    if limit is not None:
        sql += " LIMIT ?"
        params.append(int(limit))
...
def get_latest_position_ledger_event(
    conn: sqlite3.Connection,
    subject: str,
) -> dict | None:
    row = conn.execute(
        ...
        WHERE subject = ?
        ORDER BY executed_at DESC, created_at DESC, id DESC
        LIMIT 1
        """,
        (subject,),
    ).fetchone()
...
def record_panel_backfill_run(
    ...
    *,
    ...
    with conn:
        conn.execute(
            """
            INSERT INTO panel_backfill_runs (
            ...
            (
                run_id,
                _dump_json(list(subjects)),
                _dump_json(list(channels)),
                start_date,
                end_date,
                status,
                int(credit_budget),
                int(credits_spent or 0),
                int(observations_written or 0),
                _dump_json(skipped),
                _dump_json(error),
                created_at or _utc_now_iso(),
                completed_at,
...
def update_panel_backfill_run(
    conn: sqlite3.Connection,
    run_id: str,
    **updates: Any,
) -> bool:
    """Patch selected mutable backfill fields. Returns True when a row changed."""
    allowed = {
        "status",
        "credits_spent",
    ...
    if bad:
        raise ValueError(f"Unsupported panel_backfill_runs fields: {sorted(bad)}")
    if not updates:
        return False
    column_map = {
    ...
    for key, value in updates.items():
        ...
        params.append(_dump_json(value) if key in {"skipped", "error"} else value)
    params.append(run_id)
    with conn:
        cur = conn.execute(
            f"UPDATE panel_backfill_runs SET {', '.join(sets)} WHERE run_id = ?",
            params,
        )
    return cur.rowcount > 0


def _panel_backfill_run_from_row(row: sqlite3.Row | None) -> dict | None:
    if row is None:
        return None
    out = dict(row)
    out["subjects"] = _loads_json(out.pop("subjects_json"), [])
    out["channels"] = _loads_json(out.pop("channels_json"), [])
    out["skipped"] = _loads_json(out.pop("skipped_json"), [])
    out["error"] = _loads_json(out.pop("error_json"), None)
    return out


def get_panel_backfill_run(conn: sqlite3.Connection, run_id: str) -> dict | None:
    row = conn.execute(
        "SELECT * FROM panel_backfill_runs WHERE run_id = ?",
        (run_id,),
    ).fetchone()
    return _panel_backfill_run_from_row(row)


def list_panel_backfill_runs(
    conn: sqlite3.Connection,
    *,
    status: str | None = None,
    limit: int = 50,
    ...
    if status is None:
        rows = conn.execute(
            "SELECT * FROM panel_backfill_runs ORDER BY created_at DESC LIMIT ?",
            (int(limit),),
        ).fetchall()
    else:
        rows = conn.execute(
            """
            SELECT * FROM panel_backfill_runs
            WHERE status = ?
    ...
    return [_panel_backfill_run_from_row(r) for r in rows]
...
# ---------------------------------------------------------------------------
# Monitor feed + scan CRUD (SPEC F3)
# ---------------------------------------------------------------------------

def insert_monitor_feed_item(conn: sqlite3.Connection, item: dict) -> dict:
    """Insert a monitor feed item. Idempotent on item_id."""
    with conn:
        conn.execute(
...
def list_monitor_feed(
    ...
    *,
    ...
    """List monitor feed items, newest first."""
    sql = "SELECT * FROM monitor_feed WHERE 1=1"
    params: list = []
    if subject:
        sql += " AND subject = ?"
        params.append(subject)
...
def insert_monitor_scan(conn: sqlite3.Connection, scan: dict) -> dict:
    """Insert a monitor scan run record."""
    with conn:
        conn.execute(
            """INSERT OR REPLACE INTO monitor_scans
               (job_id, trigger, subjects_json, as_of_date, status,
                subjects_scanned, credits_spent, credits_budget,
                skipped_json, feed_items_generated, error, created_at, completed_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                scan["job_id"],
                scan["trigger"],
                json.dumps(scan.get("subjects") or []),
                scan["as_of_date"],
                scan.get("status", "running"),
                scan.get("subjects_scanned", 0),
                scan.get("credits_spent", 0),
                scan.get("credits_budget", 80),
                json.dumps(scan.get("skipped") or []),
                scan.get("feed_items_generated", 0),
                scan.get("error"),
                scan.get("created_at") or datetime.now(timezone.utc).isoformat(),
...
def get_monitor_scan(conn: sqlite3.Connection, job_id: str) -> dict | None:
    row = conn.execute(
        "SELECT * FROM monitor_scans WHERE job_id = ?", (job_id,)
    ).fetchone()
    if not row:
        return None
    d = dict(row)
    d["subjects"] = json.loads(d.pop("subjects_json", "[]") or "[]")
    d["skipped"] = json.loads(d.pop("skipped_json", "[]") or "[]")
    return d


def update_monitor_scan(conn: sqlite3.Connection, job_id: str, **fields) -> None:
    """Patch mutable fields on a monitor scan run."""
    allowed = {"status", "subjects_scanned", "credits_spent", "skipped",
               "feed_items_generated", "error", "completed_at"}
    sets = []
    params: list = []
    ...
    with conn:
        conn.execute(
            f"UPDATE monitor_scans SET {', '.join(sets)} WHERE job_id = ?",
            params,
        )
