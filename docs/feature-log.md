...
| M4 工作台前端 (`pricelens_mockup.html` + `api.py` REST) | 三区布局(多卡画布 + 右侧活动流 feed + 底部综合面板)+ decode/综合交互 + SSE 消费 + 复用样例卡 + 严守设计系统 + 5 REST 端点(输入校验 + 统一错误格式) | verify_m7 14 |
...
| TD6 | **AI 真实输出质量未验证**:Phase 4 验的是代码路径正确性,真实 LLM 解码/证据/综合的产出质量从未跑过;`prerun_demo.py --execute` 是首次真实 smoke | Phase 4 | 中(demo 前必验) | demo 前:用户跑 --execute(~$32)兼作真实 smoke |
