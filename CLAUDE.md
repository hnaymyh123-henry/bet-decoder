...
**Status:** **Phase 1→5 COMPLETE + Phase 4 independent code-review round DONE (2026-05-28).** Real backend M1-M5 + frontend + release scaffolding (README/LICENSE/Dockerfile) on `master` (`f667cb1`, 27 commits ahead of stale origin). **~267 assertions green** (M1 ALL · M2 30 · M3 19 · M4 15 · M5 40 · M6 39 · M7 14 · M8 35 · phase4-W1 31 · phase4-W2 25 · phase4-W3 20). **Phase 4 caught + fixed 5 CRITICAL + ~12 SHOULD-FIX real-path bugs that stub tests missed** (via 3 independent adversarial review agents): evidence cache key salted-hash→sha1 (cross-process), DCF baseline discarded→decoupled (undervalued no longer 100%-narrative), cross-thread sqlite→activity_logs now persists, live SSE→JobQueue serialized, synth theme-align O(K²T²)→capped+memoized, non-DCF strength mean=0/negative, dedup IntegrityError→optimistic-insert, ai-gate over-match, cost 3x understated, +more. **⚠ OPS:** project `.env` has a real MIROMIND_API_KEY → any decode via default hunter hits live API; run verify scripts/scripts with `MIROMIND_API_KEY=""` or rely on stubs (verify_m8 now self-protects). On Windows gbk console add `PYTHONIOENCODING=utf-8` to run verify_*/prerun without UnicodeEncodeError. **Known limitation (deferred):** band-ruler (蒙特卡洛 band 当强度尺) is dead in the real get_card path (decode_detail not persisted + traditional cards lack run_id) → synthesis uses relative-gap fallback; wiring driver-view persistence is a follow-up. **Next (needs USER):** (1) `git push` to sync origin (27 commits ahead of 82e8107); (2) before demo `python prerun_demo.py --execute` (~$32) to populate caches.
