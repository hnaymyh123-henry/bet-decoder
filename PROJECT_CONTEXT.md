...
- **下一步(需用户)**:demo 前 `python prerun_demo.py --execute`(兼首次真实 LLM smoke)—— **2026-06-01 重算诚实成本 ~$106(证据 $86.67 + flagship 市场叙事 $16.14 + 综合 $3.21),超 $100 预算**(dry-run 现已正确标 ❌ 并 exit 2)。花钱前先选杠杆:削组合逐股证据(27 调用里占 21)、减持仓数、或接受 ~$6 超支。旧"~$32"是 stale(假设每票 1 条证据 + 完全漏算叙事)。(origin 同步已于 2026-06-01 经 PR #9 完成)
