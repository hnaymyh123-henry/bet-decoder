...
- **GitHub repo**:https://github.com/hnaymyh123-henry/bet-decoder (private),master 已 push
- **Phase 1 产品对齐**:✅ 完成(5 模块 49 决策,冻结于 `PRD.md`)
- **Phase 2 技术拆解**:进行中(架构决策已锁,8 Issue 待建)
- **现有 codebase**:reverse_dcf.py / client.py / db.py(SQLite DAO,13 表 17 runs)/ api.py / pipeline.py / critic.py / short_term.py / pricelens_mockup.html + 2 张样例卡
- **待清理工程债**:旧 `pipeline.py::run_synthesizer`(被 M3 取代)· bug #34(SSE agent_step 后卡住,M5 收掉)· legacy `outputs/*.json`(~2026-06-03 删)
