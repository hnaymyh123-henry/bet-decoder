...
| 前端 | **vanilla JS**,扩展 `pricelens_mockup.html`,严守 `pricelens_design_system.md` | 沿用现有,无框架 |
...
- **GitHub repo**:https://github.com/hnaymyh123-henry/bet-decoder (private)。⚠️ **master 领先 origin ~30 提交**(本地建好,push 凭证受限 → 待用户 `git push` 同步)
- **Phase 1-5 全部完成 + Phase 4 独立 code review 完成**(2026-05-28):真后端 M1-M5 + 前端 + 发布脚手架(README/LICENSE/Dockerfile)。**~267 断言全绿**(M1-M8 + phase4-W1/W2/W3)。Phase 4 抓修 5 CRITICAL + ~12 SHOULD-FIX 真实路径 bug。
- **完整模块**:db.py(M1)· decoder.py + evidence.py(M2)· synthesizer.py(M3)· activity.py + api.py(M5/SSE+REST)· pricelens_mockup.html(M4)· prerun_demo.py(demo 预跑)· verify_m1..m8 + verify_phase4_w1/w2/w3
...
- **下一步(需用户)**:① `git push` 同步 GitHub ② demo 前 `python prerun_demo.py --execute`(~$32,兼首次真实 LLM smoke)
